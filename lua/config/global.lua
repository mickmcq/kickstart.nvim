...
-- ft specific plugins (except quarto/markdown where we want bullet continuation)
vim.api.nvim_create_autocmd({ 'BufEnter' }, {
  callback = function(_)
    local ft = vim.bo.filetype
    vim.opt.formatoptions:remove 'c'
    if ft ~= 'quarto' and ft ~= 'markdown' then
      vim.opt.formatoptions:remove 'r'
      vim.opt.formatoptions:remove 'o'
    end
  end,
...
-- conceallevel is managed per-filetype by plugins
